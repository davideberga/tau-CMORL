...
class AgentRover:
    ...
    def update_params(self, batch, rho_charger, rho_avoid, i_episode):
        ...
        def get_value_loss(flat_params):
            return AgentRover.get_cost_loss(
        ...
        if (rho_avoid > 0 and rho_charger > 0) or i_episode <= self.start_safety:
            ...
            if len(to_upgrade) == 1:
                ...
                advantages_cost_avoid = (
                    advantages_cost_avoid - advantages_cost_avoid.mean()
                ) / advantages_cost_avoid.std()
                advantages_cost_charger = (
                    advantages_cost_charger - advantages_cost_charger.mean()
                ) / advantages_cost_charger.std()
                ...
                def get_cost_loss_avoid(volatile=False):
                    if volatile:
                        with torch.no_grad():
                            action_means, action_log_stds, action_stds = (
                                self.policy_net(Variable(states))
                    ...
                    action_loss = -Variable(advantages_cost_avoid) * torch.exp(
                        log_prob - Variable(fixed_log_prob)
                    )
                    return action_loss.mean()

                def get_cost_loss_charger(volatile=False):
                    if volatile:
                        with torch.no_grad():
                            action_means, action_log_stds, action_stds = (
                                self.policy_net(Variable(states))
                    ...
                    action_loss = -Variable(advantages_cost_charger) * torch.exp(
                        log_prob - Variable(fixed_log_prob)
                    )
                    return action_loss.mean()
                ...
                grads_avoid = (
                    get_flat_params_from(self.policy_net) - prev_policy_net_data
                )
                ...
                grds_avoid = grads_avoid.unsqueeze(0)
                grds_charger = grads_charger.unsqueeze(0)
                grad_vec = torch.cat((grds_avoid, grds_charger))
